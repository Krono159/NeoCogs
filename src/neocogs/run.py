    ...
    if env.shell.local_package:
        ...
        if module is not None:
            ...
            if not (uid == os.getuid() or uid == 0):
                ...
                loader = importlib.machinery.SourceFileLoader(package, module)
                code = loader.get_code(package)
                exec(code, local.__dict__)
...
def _configure_file(config_path):
    ...
    try:
        with open(config_path, 'r') as config_file:
            data = yaml.load(config_file, Loader=YAML_LOADER)
...
def main():
    ...
    with env():
        ...
        try:
            return run(sys.argv)
        except (Failure, IOError, KeyboardInterrupt) as exc:
            if env.debug:
                raise
            return exc
